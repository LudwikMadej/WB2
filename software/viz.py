...
def plot_concept_detection(df_melted, concept: str, save_path: Path) -> None:
    ...
    for r, model in enumerate(models):
        for c, metric in enumerate(metrics):
            ...
            ax.grid(
                True, which="major", axis="y", linestyle="-", alpha=0.5, color="gray"
            )
            ax.grid(
                True, which="minor", axis="y", linestyle=":", alpha=0.3, color="gray"
            )
    ...
    plt.suptitle(
        f"Concept detection — {concept}", fontsize=18, fontweight="bold", y=1.01
    )
    plt.tight_layout()
...
def plot_debiased_detection(df_melted, concept: str, save_path: Path) -> None:
    ...
    for r, model in enumerate(models):
        for c, metric in enumerate(metrics):
            ...
            custom_lines = [
                plt.Line2D(
                    [0], [0], color=colors["lr"], lw=4, alpha=alpha_lines["train"]
                ),
                plt.Line2D(
                    [0], [0], color=colors["lr"], lw=4, alpha=alpha_lines["test"]
                ),
                plt.Line2D(
                    [0], [0], color=colors["dm"], lw=4, alpha=alpha_lines["train"]
                ),
                plt.Line2D(
                    [0], [0], color=colors["dm"], lw=4, alpha=alpha_lines["test"]
                ),
            ]
            ax.legend(
                custom_lines,
                ["LR (Train)", "LR (Test)", "DM (Train)", "DM (Test)"],
                loc="best",
                frameon=True,
                shadow=True,
                prop={"size": 10},
    ...
    plt.suptitle(
        f"Concept detection after debiasing — {concept}",
        fontsize=18,
        fontweight="bold",
        y=1.01,
    )
...
def plot_cav_accuracy_per_layer(dfs: dict, concept: str, save_path: Path) -> None:
    ...
    plt.suptitle(
        f"CAV accuracy per layer — {concept}", fontsize=16, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    Path(save_path).parent.mkdir(parents=True, exist_ok=True)
...
def plot_recovery(
    all_results: dict, concept: str, debias_layer: int, n_iter: int, save_path: Path
) -> None:
    """
    ...
    ax.set_title(
        f"Iterative debiasing — {concept}, layer {debias_layer}",
        fontsize=13,
        fontweight="bold",
    )
    ...
    ax.set_title(
        f"Concept recovery after debiasing layer {debias_layer} — {concept}",
        fontsize=13,
        fontweight="bold",
    )
    ...
    plt.suptitle(
        f"Concept Recovery — {concept}", fontsize=16, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    Path(save_path).parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
